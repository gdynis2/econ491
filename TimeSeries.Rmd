...
train_df = head(df_TX_ts_train, 1913 - 28)
test_df = tail(df_TX_ts_train, 28)
...
plot_df_errors = data.frame(date = as.Date(rownames(test_df)), errors = errors, squared_errors = squared_errors)
...
total_sales_ts <- ts(train_df$total_sales, frequency = 365)  # Adjust frequency based on your data (e.g., 7 for weekly, 365 for daily)
...
```{r, include=TRUE}
#### RMSE Calculation ####
test_exogenous_vars <- test_df[, setdiff(names(test_df), c("total_sales", "weekday_Sunday"))]

forecast_values <- forecast(model, xreg = as.matrix(test_exogenous_vars), h = nrow(test_df))

predicted_values <- as.numeric(forecast_values$mean)
actual_values <- test_df$total_sales
